...
and a I to the root object is returned. In case an I/O error
occurs while reading, C is returned instead. Other serious
...
connected. The routines to call have an initial C prefix for I,
as in C and C. At retrieval time, your data will be
...
The C and C routine are equivalent to
C and C, except that they get an exclusive lock on
the file before writing.  Likewise, C does the same
as C, but also gets a shared lock on the file before reading.

As with any advisory locking scheme, the protection only works if you
systematically use C and C.  If one side of
your application uses C whilst the other uses C,
...
the help of L. To enable this feature, set
C<$Storable::Deparse> to a true value. To enable deserializazion,
C<$Storable::Eval> should be set to a true value. Be aware that
deserialization is done through C, which is dangerous if the
Storable file contains malicious data. You can set C<$Storable::Eval>
to a subroutine reference which would be used instead of C. See
below for an example using a L compartment for deserialization
of CODE references.
...
Storable will attempt to do the right thing, by Cing if it
...
Perl encounters a utf8 value it cannot represent, it will C.
To change this behaviour so that Storable deserializes utf8 encoded
values as the string of bytes (effectively dropping the I flag)
set C<$Storable::drop_utf8> to some C value.  This is a form of
...
all values unlocked.  To make Storable C instead, set
C<$Storable::downgrade_restricted> to a C value.  To restore
the default set it back to some C value.
...
C<$Storable::accept_future_minor> to some C value.
...
Normal errors are reported by having store() or retrieve() return C.
...
I is true when we're part of a deep clone operation, I
is the serialized string you returned to the engine in C,
and there may be an optional list of references, in the same order you gave
them at serialization time, pointing to the deserialized objects (which
have been processed courtesy of the Storable engine).

When the Storable engine does not find any C hook routine,
it tries to load the class by requiring the package dynamically (using
the blessed package name), and then re-attempts the lookup.  If at that
time the hook cannot be located, the engine croaks.  Note that this mechanism
will fail if you define several classes in the same file, but L
warned you.

It is up to you to use this information to populate I the way you want.
...
there's a utility called C, which recognizes data files based on
their contents (usually their first few bytes).  For this to work,
a certain file called F needs to taught about the I
of the data.  Where that configuration file lives depends on the UNIX
flavour; often it's something like F or
F.  Your system administrator needs to do the updating of
the F file.  The necessary signature information is output to
STDOUT by invoking Storable::show_file_magic().  Note that the GNU
implementation of the C utility, version 3.38 or later,
...
It won't work across a sequence of C and C operations,
...
C is a blunt tool.  There is no facility either to
return B strings as utf8 sequences, or to attempt to convert utf8
data back to 8 bit and C if the conversion fails.
